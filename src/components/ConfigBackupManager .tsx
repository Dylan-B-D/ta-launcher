...
const ConfigBackupManager = () => {
  ...
  const createBackup = async () => {
    ...
    const duplicate = backups.some((backup) =>
      selectedFiles.some((file) => backup.name === `${name}_${file}`)
    );
  ...
  const confirmLoadBackup = (backupName: string) => {
    setConfirmationModal({
      ...
      message:
        "Are you sure you want to load this backup? This will overwrite the current configuration.",
    });
  };
                ...
                <Card shadow="sm" padding="sm">
                  ...
                  <Group grow align="right" mt="xs">
                    <Button
                      ...
                      variant="light"
                      size="xs"
                      onClick={() => confirmLoadBackup(name)}
                      style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.15)" }}
                    >
                      Load
                    </Button>
                    <Button
                      ...
                      size="xs"
                      color="red"
                      onClick={() => confirmDeleteBackup(name)}
                      style={{ boxShadow: "0 4px 8px rgba(0, 0, 0, 0.15)" }}
                    >
                      Delete
                    </Button>
