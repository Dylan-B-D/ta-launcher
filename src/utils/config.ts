...
export async function saveConfig(config: Record<string, any>) {
  await writeTextFile(CONFIG_FILE, JSON.stringify(config), {
...
export const loadConfig = async (
  ...
  try {
    ...
    const savedConfig = await readTextFile(CONFIG_FILE, {
      baseDir: BaseDirectory.AppLocalData,
    });
    if (savedConfig) {
...
export async function loadDownloadedPackages(): Promise<Map<string, string>> {
  try {
    const fileExists = await exists(DOWNLOADED_PACKAGES_FILE, {
      baseDir: BaseDirectory.AppLocalData,
    });

    if (!fileExists) {
      console.log(
        "Downloaded packages file not found, this is expected on the first launch."
      );
      return new Map();
    }

    const savedPackages = await readTextFile(DOWNLOADED_PACKAGES_FILE, {
      baseDir: BaseDirectory.AppLocalData,
    });
