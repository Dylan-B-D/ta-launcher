...
export const DownloadProvider: React.FC<DownloadProviderProps> = ({
  ...
  const calculateOverallProgress = useCallback(() => {
    return Array.from(progressMapRef.current.values()).reduce(
      (sum, value) => sum + value,
      0
    );
  }, []);
  ...
  const checkPackageHashes = (
    savedPackages: Map<string, string>,
    currentPackages: Packages,
    toUpdate: Set<string> = new Set()
  ) => {
  ...
  useEffect(() => {
    ...
    const initializePackages = async () => {
      ...
      const packagesToUpdate = Array.from(
        checkPackageHashes(savedPackages, packages)
      );
    ...
    const unlistenCompleted = listen(
      ...
      async (event: any) => {
        ...
        setCompletedPackages((prev) => {
          ...
          const updatedPackagesToUpdate = Array.from(
            checkPackageHashes(newMap, packages)
          );
