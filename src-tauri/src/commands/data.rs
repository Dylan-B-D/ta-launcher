...
pub const CONFIG_FILES: [ConfigFileInfo; 4] = [
    ConfigFileInfo {
        name: "tribes.ini",
        default_path: "../public/configs/defaultini/tribes.ini",
    }, // Default but: Disabled Vsync, motion blur, and set force static terrain to true
    ConfigFileInfo {
        name: "TribesInput.ini",
        default_path: "../public/configs/defaultinput/TribesInput.ini",
    }, // Default but: Reduced mouse sensitivity to 10 from 30
    ConfigFileInfo {
        name: "TribesHelpText.ini",
        default_path: "../public/configs/defaulttribeshelptext/TribesHelpText.ini",
    }, // Disabled help text by default
    ConfigFileInfo {
        name: "TribesUser.ini",
        default_path: "../public/configs/defaulttribesuser/TribesUser.ini",
    }, // Default
];
...
pub static LOGIN_SERVER_PUG_DETAILS: Lazy<String> =
    Lazy::new(|| format!("http://{}:9080/detailed_status", LOGIN_SERVER_PUG));

pub static LOGIN_SERVER_COMMUNITY_DETAILS: Lazy<String> =
    Lazy::new(|| format!("http://{}:9080/detailed_status", LOGIN_SERVER_COMMUNITY));
...
pub fn get_game_path(handle: &AppHandle) -> Result<PathBuf, String> {
    ...
    let game_path_str = config_json["gamePath"]
        .as_str()
...
pub fn get_game_folder(handle: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(game_path
        .parent()
        .expect("Failed to get game folder")
        .to_path_buf())
}
...
pub fn get_tribes_dir(handle: &AppHandle) -> Result<PathBuf, String> {
    ...
    Ok(game_folder
        .parent()
        .expect("Failed to get Binaries folder")
        .parent()
        .expect("Failed to get parent of Binaries")
        .to_path_buf())
}
...
pub fn get_original_dlls_dir(handle: &AppHandle) -> Result<PathBuf, String> {
    match handle
        .path()
        .resolve("../public/originalDLLs", BaseDirectory::Resource)
    {
        Ok(path) => Ok(path),
