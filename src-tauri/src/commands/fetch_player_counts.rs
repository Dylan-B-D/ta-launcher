...
async fn fetch_player_data(url: &str) -> Result<(usize, Vec<String>), String> {
    ...
    let json: Value = serde_json::from_str(&text).map_err(|e| format!("Failed to parse JSON: {}", e))?;

    let players = json["online_players_list"].as_array()
        .map_or(vec![], |list| list.iter().filter_map(|p| p.as_str().map(|s| s.to_string())).collect());
